...
# --- PESAN TERSEMAT DARI GURU (VERSI KOMPAK) ---
    ...
    if st.session_state.menu_pilihan == 'siswa':
        ...
        with tab_kalender:  # --- TAB KALENDER (KONTRAS TINGGI) ---
            # --- TAB/MENU KALENDER AKADEMIK (VERSI PDF VIEWER) ---
            st.markdown("### 🗓️ Kalender Akademik & Jadwal Fisika XII")
            st.write("Silakan simak atau unduh kalender akademik pembelajaran Fisika XII di bawah ini:")
            
            # 1. LINK GOOGLE DRIVE PDF KALENDER BAPAK
            url_pdf_kalender = "https://drive.google.com/file/d/1CSypvaXOorN33j8s6tuOlR1u6f04HDUh/view?usp=share_link/view?usp=sharing"
            
            # Konversi link Google Drive agar bisa ditayangkan langsung di dalam frame (embed preview)
            if "drive.google.com" in url_pdf_kalender:
                url_embed = url_pdf_kalender.replace("/view?usp=sharing", "/preview").replace("/view", "/preview")
            else:
                url_embed = url_pdf_kalender
            
            # 2. TAMPILAN PDF EMBEDDED
            st.markdown(
                f'<iframe src="{url_embed}" width="100%" height="600" style="border: 2px solid #1A365D; border-radius: 10px;"></iframe>', 
                unsafe_allow_html=True
            )
            
            st.write(" ")
            
            # 3. TOMBOL UNDUH / BUKA TAB BARU
            col_k1, col_k2, col_k3 = st.columns([1, 2, 1])
            with col_k2:
                st.link_button(
                    label="📥 Buka / Unduh File PDF Kalender",
                    url=url_pdf_kalender,
    ...
    # --- PORTAL GURU (SOLUSI FIX CSS INPUT & QUERY SUPABASE) ---
        ...
        if kode == "CERIA2024":
            ...
            res_data = supabase.table("laporan_kehadiran") \
                ...
                .lte("jadwal_kbm.tanggal", t_akhir.isoformat()) \
                .execute()
            ...
            if res_data.data:
                ...
                today_str = datetime.date.today().strftime('%Y-%m-%d')
                data_hari_ini = [r for r in res_data.data if str(r.get('jadwal_kbm', {}).get('tanggal')) == today_str]
                                ...
                                for i in range(1, 7):
                                    ...
                                    if progres:
                                        data_tabel.append({
                                            "Klp": i,
                                            "Status": "✅",
                                            "Progress": f"{progres.get('persentase', 0)}%",
